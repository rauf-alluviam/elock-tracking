  ...
  try {
    ...
    if (search) {
      ...
      filteredJobs = jobs.filter(item =>
        (item.container_number && item.container_number.toLowerCase().includes(lowerSearch)) ||
        (item.vehicle_no && item.vehicle_no.toLowerCase().includes(lowerSearch)) ||
        (item.driver_name && item.driver_name.toLowerCase().includes(lowerSearch)) ||
        (item.tr_no && item.tr_no.toLowerCase().includes(lowerSearch)) ||
        (item.elock_no && item.elock_no.toLowerCase().includes(lowerSearch)) ||
        (item.consignor?.name && item.consignor.name.toLowerCase().includes(lowerSearch)) ||
        (item.consignee?.name && item.consignee.name.toLowerCase().includes(lowerSearch))
    ...
    const transformedData = filteredJobs.map(item => ({
      ...
      ...Object.keys(item).reduce((acc, key) => {
        if (!['_id', 'tr_no', 'container_number', 'vehicle_no', 'driver_name', 'driver_phone', 'elock_no', 'elock_assign_status', 'consignor', 'consignee', 'goods_pickup', 'goods_delivery', 'seal_no', 'gross_weight', 'net_weight', 'createdAt', 'updatedAt'].includes(key)) {
          acc[key] = item[key];
        }
        return acc;
      }, {})
