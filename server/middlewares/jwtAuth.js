 ...
 * Token can be in multiple places:
 * 1. URL query parameter: ?token=xxx
 * 2. Authorization header: Bearer xxx
 * 3. Cookies: access_token, exim_sso_token, or token
 *
...
export const verifyToken = (req, res, next) => {
  try {
    ...
    // Check cookies (multiple cookie names for cross-module compatibility)
    else if (req.cookies) {
      // Priority: access_token (from client module) > exim_sso_token > token
      const cookieToken =
        req.cookies.access_token ||
        req.cookies.exim_sso_token ||
        req.cookies.token;
      if (cookieToken) {
    ...
    if (!token) {
      ...
      if (process.env.SSO_ENABLED === "true") {
        ...
        console.log(
          "⚠️ No token provided, but SSO not enforced. Allowing access."
        );
        next();
        return;
      }
    ...
    // Log user access
    console.log(
      `✅ Authenticated user with IE Code: ${decoded.ie_code_no} (token from ${tokenSource})`
    );
